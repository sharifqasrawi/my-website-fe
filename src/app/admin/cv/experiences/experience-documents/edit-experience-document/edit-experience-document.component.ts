...
@Component({
  selector: 'app-edit-experience-document',
  templateUrl: './edit-experience-document.component.html',
  styleUrls: ['./edit-experience-document.component.css']
})
export class EditExperienceDocumentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public dialogRef: MatDialogRef<EditExperienceDocumentComponent>,
  ...
  ngOnInit(): void {
    ...
    this.store.select('experiences').subscribe(state => {
      ...
      if (this.data.docId && this.editMode) {
        const doc = state.experiences.find(e => e.id === this.data.experienceId).documents.find(d => d.id === this.data.docId);
  ...
  onSubmit() {
    ...
    if (!this.editMode) {
      this.store.dispatch(new ExperiencesActions.CreateDocStart({
      ...
      this.store.dispatch(new ExperiencesActions.UpdateDocStart({
        id: this.data.docId,
        name_EN: this.form.value.name_EN,
        name_FR: this.form.value.name_FR,
        description_EN: this.form.value.description_EN,
